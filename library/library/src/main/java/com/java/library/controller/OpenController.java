...
//		session.setAttribute("origin", ImgList);
	...
	public String boardDelete(@RequestParam Map<String, Object> paramMap, HttpSession session) {
		...
		return JSONObject.fromObject(JSONSerializer.toJSON(bsi.boardDelete(paramMap, session))).toString();
	}
	...
	@ResponseBody
	public String postBoardUpdate(@RequestParam Map<String, Object> paramMap, HttpSession session) {
		// JSONSerializer => MAP은 순서가 없어서 디비에 저장된 값 순서대로 뽑아 쓰기...위해서..(?)
		return JSONObject.fromObject(JSONSerializer.toJSON(bsi.boardUpdate(paramMap, session))).toString();
	}
	

	// 부분전환(ajax를 이용한 jsp파일 불러오기)
	@RequestMapping(value = "changemain.do", method = RequestMethod.POST)
	public ModelAndView ajaxChangeMain(ModelAndView mav, @RequestParam Map<String, Object> paramMap, HttpSession session) {
		...
		/** 게시판 데이터 수정 부분(화면부분) */
			...
			if(nProfile != null) {
				...
				rstMap = new HashMap<String, Object>();
				rstMap.put("result", "로그인 후 이용 가능합니다.");
				mav = HttpUtil.returnJson(rstMap);
			}
//				rstMap = new HashMap<String, Object>();
//				mav.addObject("msg", "")
