...
<head>
	...
	var pathList = [
					{path:"list", run: function() {board.createBoardList();}},
					{path:"search", run: function() {data.createhtmlSearch();}},
					{path:"borrow", run: function() {book.createBookLibraryList();}} 
// 					{path:"newbook", run: () => {data.initDataNewbook();}}
				   ];
	var util = {
			// 페이징 번호 화면 생성
			createHtmlPaging : function(){
// 				console.log(totCnt);
// 				console.log(viewRow);
				// 총 페이지 갯수
				var paging = totCnt / viewRow;
				paging = paging > (100 / viewRow) ? (100 / viewRow) : paging;
				...
				$(".page-item a").off().on("click", function(){ 
				   ...
				   // 해당 페이지 번호를 눌렀을 때  그 화면의 처음 행 인덱스 구하기
				   start = (viewRow * (page - 1)); // 20 - 10 = 10
				   
				   var path = location.pathname.substring(location.pathname.lastIndexOf("/") + 1, location.pathname.lastIndexOf("do") - 1);
				   for(var i = 0; i < pathList.length; i++){
					   if(path == pathList[i].path){
						   pathList[i].run();
					   }
				   }
