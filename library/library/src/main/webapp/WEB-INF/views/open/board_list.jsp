...
<div class="location">
	<p class="main_p">
		<a href=""><img src="${path}/resources/img/icons/homeicon.PNG"
			style="height: 20px" alt="" /></a> &gt; <a href="#">열린마당</a>
		&gt; 자유게시판
	</p>
	<!-- start: 신간안내-->
	<!-- start: 신간안내-->
		...
		<div>
			<!-- end: 자유게시판-->
		    	<c:if test="${empty UserSession}">
					<button type="button" id="board_write" class="btn btn-default text-white bg-redred writbtn3 boardInput_button hide"
						onclick="javascipt:board.writeform();">쓰기</button>
				</c:if>	
				<c:if test="${!empty UserSession}">
					<button type="button" id="board_write" class="btn btn-default text-white bg-redred writbtn3 boardInput_button"
						onclick="javascipt:board.writeform();">쓰기</button>
				</c:if>
		</div>
	</div>
